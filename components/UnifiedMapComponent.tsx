...
const UnifiedMapComponent: React.FC<UnifiedMapComponentProps> = ({
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    // Fallback to default center (Caracas)
    ...
    if (!apiKey) {
        return (
            <div className={`relative w-full h-full ${className} z-0 bg-gray-100 flex items-center justify-center`}>
                <div className="text-center p-6">
                    <AlertCircle size={48} className="text-red-500 mx-auto mb-4" />
                    <h3 className="text-lg font-bold text-gray-900 mb-2">Configuración Pendiente</h3>
                    <p className="text-sm text-gray-600">
                        La API Key de Google Maps no está configurada. Por favor, contacta al administrador.
                    </p>
                </div>
            </div>
        );
